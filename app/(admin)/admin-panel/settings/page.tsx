    ...
    <div className="p-3 md:p-8 space-y-4 md:space-y-6 max-w-7xl mx-auto pb-32 bg-transparent text-white font-sans text-left text-xs md:text-sm animate-in fade-in duration-300">
          ...
          <div className="space-y-3">
            ...
            <form onSubmit={handleSaveResendSettings} className="bg-neutral-950/30 backdrop-blur-md border border-neutral-800 rounded-2xl p-5 space-y-4 shadow-lg">
              ...
              <div className="space-y-1.5 text-left">
                ...
                />
                <p className="text-[9px] text-neutral-600 font-bold mt-1">Email pengirim terverifikasi (default: onboarding@resend.dev)</p>
              </div>
      ...
      <div className="pt-4">
        <button 
          ...
          className="w-full py-4 bg-red-500/10 text-red-500 border border-red-500/20 rounded-xl text-xs font-bold tracking-widest flex items-center justify-center gap-2 hover:bg-red-500 hover:text-white transition-all active:scale-95 duration-300 cursor-pointer"
        >
...
function SettingItem({ icon, title, value, isLink }: { icon: ReactNode, title: string, value: string, isLink?: boolean }) {
  return (
    <div className="flex items-center justify-between p-4 hover:bg-neutral-900/40 transition-all duration-300">
      ...
      {isLink && <span className="text-yellow-500 text-[10px] font-black tracking-widest hover:text-yellow-400 transition-colors duration-300">Edit</span>}
    </div>
  )
}
